...
- [ ] Link test results to ZenTao task via `zentao task comment --task <id> --comment "Tests: <pass/fail>"`
- [ ] If bug-related: create ZenTao bug via `zentao bug create --product 8 --title "<bug>" --severity 3` and link reproduction test
- [ ] Notify team of regression suite results via MIM (`winpeek_mim_send`)
- [ ] Share coverage report with team via MIM if coverage threshold is met
- [ ] Reference ZenTao story/task ID in test commit messages
