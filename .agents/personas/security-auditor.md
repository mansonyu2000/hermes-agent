...
When operating within the Hermes agent framework, the following actions are automatically performed:

- [ ] Create ZenTao bugs for Critical/High findings via `zentao bug create`
- [ ] Add security notes to ZenTao task via `zentao task comment`
- [ ] Alert security channel via MIM if Critical finding is confirmed
