...
已验证的 UIA 采集路径:
  1. 搜索 → search_list (search_item_<名>)
  2. 资料卡 → 点当前联系人名 → 读 Profile 面板
  3. 消息全量 → chat_message_list 滚动 + 读取

用法:
  python wechat_profile.py                  # 侦察当前聊天
  python wechat_profile.py --search "于杨敏"  # 搜索联系人
  python wechat_profile.py --profile         # 打开资料卡
    ...
    for scroll_i in range(max_scrolls):
        ...
        for item in msg_list.GetChildren():
            try:
                ...
                key = (name[:40], r.top)  # 相同文本+相同Y = 同一条
                if key in seen_keys:
                    continue
                seen_keys.add(key)
        ...
        print(f"  [{scroll_i+1}] +{new_in_round} 条, 累计 {len(all_msgs)}")
