"""
wechat_api.py — 微信自动化三层架构

🧠 LLM → 👁️ eyes 看 → 决策 → 🤖 hands 执行 → 💾 engine 入库
每层只做一件事。输入→输出，无副作用（除 hands 层）。
...
from rpa_tools import parse_session_item
...
class WeChatEyes:
    ...
    def get_sessions(self):
        ...
        for item in sl.GetChildren():
            try:
                ...
                if not info["name"] or info["name"] in ('群聊', '消息免打扰'): continue
                ...
                pass
        out.sort(key=lambda x: x["rect"][1])  # 按 Y 排序
        return out
    ...
    # 🚪 门分类 — 点击可展开/折叠, 门内有子项可采集
    _DOORS_ENTER = {'服务号', '群聊', '折叠的聊天', '群助手', '微信游戏'}  # 进门采集
    _DOORS_SKIP = {'公众号'}   # 公众号是文章链接, 暂不采集
    _WRONG_DOORS = _DOORS_ENTER | _DOORS_SKIP  # 兼容旧引用

    def is_right_place(self):
        """验证在微信店(一级菜单)中 → {ok, reason, door_sign, door_rect, first_names}

        ok=False: 在聊天窗口 / session_list不存在
        ok=True:  在微信店 (仅大厅OK)
        门头不在session_list中时无法检测, 靠调用方先点微信nav保证在大厅
        """
        sl = self.w.Control(AutomationId='session_list')
        if not sl.Exists():
            if self.w.Control(AutomationId='chat_message_list').Exists():
                return {"ok": False, "reason": "在聊天窗口", "door_sign": None, "door_rect": None, "first_names": []}
            return {"ok": False, "reason": "session_list不存在", "door_sign": None, "door_rect": None, "first_names": []}
        ...
        if not first_names:
            return {"ok": False, "reason": "无可见会话", "door_sign": None, "door_rect": None, "first_names": []}

        if door_sign:
            return {"ok": True, "reason": f"微信店·门「{door_sign}」", "door_sign": door_sign, "door_rect": door_rect, "first_names": first_names[:5]}
        return {"ok": True, "reason": "微信店·大厅", "door_sign": None, "door_rect": None, "first_names": first_names[:5]}
    ...
    def find_session(self, name):
        """在可见列表中按名找会话 → {name, aid, rect} or None"""
    ...
    def find_by_aid(self, aid):
        """按 AutomationId 在session_list中定位 → rect or None"""
        sl = self.w.Control(AutomationId='session_list')
...
class WeChatHands:
    ...
    # ── 启动自校准 (TECHNICAL.md §8.2) ──

    def calibrate(self, log_func=None):
        """启动自检: 测所有关键点位 → {点位名: ok/fail, ...}"""
        ...
        points = {
            ...
            "搜索框":  (wr.left + 190, wr.top + 60),
            "导航微信": (wr.left + 30, wr.top + 114),
            "导航通讯录": (wr.left + 30, wr.top + 162),
            "最小化":  (wr.right - 100, wr.top + 16),
            "关闭":    (wr.right - 20, wr.top + 16),
            "消息框":  (wr.left + wr.width() // 3, wr.bottom - 60),
        ...
        points[f"中间行({mid_idx})"] = (wr.left + self._LIST_CX, mid_y)
    ...
    def scroll_sessions(self, amount=-2000):
        """会话列表滚轮 → 鼠标移到列表区, 不点击, 直接滚 (点中会话反而坏事)"""
        sl = self.w.Control(AutomationId='session_list')
    ...
    def scrollbar_nudge_up(self, rows=5, debug_func=None):
        """拖滑块上移 ~rows 行（活塞一步）→ (changed, before_top_y, after_top_y)
        在 session_list 控件内操作, 不碰窗口边界
        """
        debug = debug_func or (lambda m: None)
        sl = self.w.Control(AutomationId='session_list')
        ...
        track_top = r.top + 10
        track_bottom = r.bottom - 10           # session_list 控件内, 不碰窗口
        ...
        sb_x = r.right - 5
        start_y = track_bottom                 # 沉底后滑块在轨道底
    ...
    def focus_msg_area(self):
        """点聊天区获焦点, 让 PageUp 生效 → bool
        聊天区中心: (窗口左+宽*2/3, 窗口底-60)  ← 右侧聊天面板
        """
        wr = self.w.BoundingRectangle
    ...
    def right_click_menu(self, rect, menu_text, match_mode="contains"):
        """右键点击 → 弹出菜单 → 点击菜单项 → bool
        rect: 右键点击位置 (x,y,w,h)
        menu_text: 菜单项文字, 如 '移出「折叠的聊天」'
        """
        x, y, w, h = rect
...
from rpa_tools import parse_chat_time as _parse_msg_time
...
class Brain:
    ...
    def _default_answer(self, template_name, answer_format):
        ...
        if answer_format == "json":
            if template_name == 'is_right_door':
                return {"door": "不确定", "action": "unsure", "reason": "无LLM"}
            return {}
    ...
    def prioritize(self, sessions):
        """🎯 决策: 给定会话列表, 返回处理顺序。
        当前用机械规则(免费). 切 LLM: return self.ask('prioritize_sessions', sessions=str(sessions))"""
        # 机械规则: 未读>置顶>未读数
        return sorted(sessions,
            key=lambda s: (s.get('unread', 0) > 0, s.get('is_pinned', False), s.get('unread', 0)),
            reverse=True)
        # 切 LLM: return self.ask('prioritize_sessions', sessions=json.dumps(sessions, ensure_ascii=False))
