...
流程: 搜索联系人 → 打开对话 → 滚顶读资料 → 滚消息 → 入库

用法:
  python wechat_collect.py --name "于杨敏" --full    # 全量采集
  python wechat_collect.py --name "于杨敏" --inc      # 增量采集
  python wechat_collect.py --name "于杨敏" --profile  # 只采资料
"""
...
from wechat_uia import WeChatUIA, AID
...
def parse_profile_text(raw_text):
    ...
    if lines:
        ...
        if first and not first.startswith("微信号"):
            info["nickname"] = first
...
def collect_profile(wx, db, contact_name):
    ...
    print(f"   💾 已入库 friend_id={fid}  wxid={info.get('wxid','?')}  nickname={info.get('nickname','?')[:30]}")
    if info.get("phone"):
...
def collect_messages(wx, db, friend_wxid, my_wxid=None, max_pages=80, incremental=True):
    ...
    算法:
      1. 查本地最新时间 T_local
      2. PageDown 到底 → 看最新时间 T_wechat
      3. PageUp 往上翻, 所有记录存缓存
      4. 遇到日期 < T_local → 停止 (已采集过)
      5. 缓存按时间正序(旧→新)批量写入本地库
    """
    if my_wxid is None:
        my_wxid = db.wxid
    ...
    for page in range(max_pages):
        ...
        for item in msg_list.GetChildren():
            try:
                ...
                if is_date:
                    current_time = text
                    # 停止条件: ISO 时间比对 (last_local_time 是 ISO 格式)
                    if last_local_time:
                ...
                # msg_ts: 从中文日期生成 ISO 时间戳 (用于排序 + 增量比对)
                ...
                msg_data = {
                    ...
                    "sender_name": "我" if is_bubble else friend_wxid,
        ...
        if stopped_by_time:
            print(f"   [{page+1}] +{new_in_page} 条缓存 → 遇到已采集时间, 停止翻页")
            break

        print(f"   [{page+1}] +{new_in_page} 条缓存, 累计 {len(buffer)}, 时间: {current_time}")
...
def collect_all(wx, db, contact_name, incremental=False):
    ...
    print(f"\n{'='*50}")
    s = db.stats()
    print(f"数据概况: {s['friends']}联系人 {s['messages']}消息 {s['files']}附件")
...
def main():
    ...
    parser.add_argument("--profile", action="store_true", help="只采集资料")
